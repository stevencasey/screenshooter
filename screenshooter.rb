...
def upload(png_file)
  ...
  AWS::S3::S3Object.store("ss/#{png_file.gsub('__', '/')}", open("#{SAVE_DIR}#{png_file}"), s3_config['bucket'], :access => :public_read, 'Cache-Control' => "public, max-age=60")
  "http://#{s3_config['bucket']}.s3.amazonaws.com/ss/#{png_file.gsub('__', '/')}"
end
